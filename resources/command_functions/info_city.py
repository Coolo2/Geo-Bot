...
async def get_city_info_embed(client : client.Client, city : geography.City, lp):
    ...
    city.get_country(client.all_countries)
    ...
    populationPercent = round((city.population / city.country.population) * 100, 2) if city.country else 100
    ...
    embed.add_field(name=lp.population, value=f"{city.population:,d} [`{populationPercent}%`] ({lp.With(city.get_population_rank(client.cities)).rank})")
