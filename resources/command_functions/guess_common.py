...
from resources.command_functions import info_user, info_country
...
def calculate_guess_game_result(client: client.Client, user : discord.User, game : games.CountryGuessGame):
    ...
    if timeSinceStart <= 20:
        points_time = round(20 - timeSinceStart)
...
async def end_game(
    ...
    if user == None and interaction.user.id != game.starter.id:
        return await interaction.response.send_message(ephemeral=True, content=lpr.didntStartGame)
    ...
    newGameView = NewGameView(client, start_game_callback, answer)
    ...
    for child in newGameView.children:
        ...
        if child.label == lang.default.getCountryInfo:
            child.label = lp.getCountryInfo
...
class NewGameView(discord.ui.View):
    def __init__(self, client : client.Client, start_game_callback : Coroutine, answer : geography.Country):
        self.client = client
